/**
 * @file GameCtrl.cpp
 * Implementation of a library that communicates with the GameController.
 * It also implements the official button interface and sets the LEDs as
 * specified in the rules.
 *
 * @author Thomas Röfer (stico), Dario Albani, Marco Paolelli, Tiziano quellochefailcaffe Manoni
 */
#ifdef __clang__
...
private:
  ...
  std::chrono::time_point<std::chrono::system_clock> initialTime; /**< When the module starts. */
  std::chrono::time_point<std::chrono::system_clock> whenChestButtonStateChanged; /**< When last state change of the chest button occured (DCM time). */
  std::chrono::time_point<std::chrono::system_clock> whenChestButtonPressed; /**< When the chest button was pressed (DCM time). */
  std::chrono::time_point<std::chrono::system_clock> whenChestButtonReleased;/**< When the chest button was released (DCM time). */
  std::chrono::time_point<std::chrono::system_clock> whenLeftFootButtonStateChanged; /**< When last state change of the left foot bumper occured (DCM time). */
  std::chrono::time_point<std::chrono::system_clock> whenRightFootButtonStateChanged; /**< When last state change of the right foot bumper occured (DCM time). */
  std::chrono::time_point<std::chrono::system_clock> whenPacketWasReceived; /**< When the last GameController packet was received (DCM time). */
  std::chrono::time_point<std::chrono::system_clock> whenPacketWasSent; /**< When the last return packet was sent to the GameController (DCM time). */
  ...
  void init()
  {
    ...
    std::memset(&gameControllerAddress, 0, sizeof(gameControllerAddress));
    ...
    whenChestButtonReleased = initialTime;
    whenChestButtonStateChanged = initialTime;
    whenLeftFootButtonStateChanged = initialTime;
    whenRightFootButtonStateChanged = initialTime;
    whenPacketWasReceived = initialTime;
    whenPacketWasSent = initialTime;
  ...
  void handleOutput()
  {
        ...
        gameCtrlData.teams[1].teamNumber == teamNumber))
    {
         ...
         team.players[*playerNumber - 1].penalty != previousPenalty)
      {
        ...
        else if((std::chrono::duration<float> (now - whenPacketWasReceived)).count() < GAMECONTROLLER_TIMEOUT &&
      ...
      if((std::chrono::duration<float> (now - whenPacketWasReceived)).count() < GAMECONTROLLER_TIMEOUT &&
         (std::chrono::duration<float> (now - whenPacketWasSent)).count() >= ALIVE_DELAY &&
         send(GAMECONTROLLER_RETURN_MSG_ALIVE))
        whenPacketWasSent =  std::chrono::system_clock::now();
    }
  }
  ...
  void handleInput()
  {
    ...
    if(*teamNumberPtr != 0)
    {
      ...
      // TODO REFORMULATE WITH LOLA memory->insertData("GameCtrl/teamNumber", 0);
      init();
    }
    ...
    if(teamNumber && *playerNumber)
    {
      ...
      if(*playerNumber <= gameCtrlData.playersPerTeam)
      {
        ...
        if(chestButtonReleased && (std::chrono::duration<float> (now - whenChestButtonStateChanged)).count() >= BUTTON_DELAY)
        {
          ...
          if(chestButtonPressCounter == 0)
            whenChestButtonStateChanged = std::chrono::system_clock::now();  // reset last chest button state change to ignore the next press
        }

        if(chestButtonPressCounter > 0 &&
           (std::chrono::duration<float> (now - whenChestButtonStateChanged)).count() >= BUTTON_DELAY &&
           (std::chrono::duration<float> (now - whenChestButtonPressed)).count() < CHESTBUTTON_PRESS_DURATION)
        {
          if((std::chrono::duration<float> (now - whenChestButtonReleased)).count() >= CHESTBUTTON_TIMEOUT)
          {
            if(whenChestButtonStateChanged != initialTime  && (std::chrono::duration<float> (now - whenPacketWasReceived)).count() >= GAMECONTROLLER_TIMEOUT)  // ignore first press, e.g. for getting up
            {
        ...
        if(gameCtrlData.state == STATE_INITIAL && team.players[*playerNumber - 1].penalty == PENALTY_NONE)
        {
          ...
          if(leftFootButtonPressed != previousLeftFootButtonPressed && (std::chrono::duration<float> (now - whenLeftFootButtonStateChanged)).count() >= BUTTON_DELAY)
          {
          ...
          if(rightFootButtonPressed != previousRightFootButtonPressed && (std::chrono::duration<float> (now - whenRightFootButtonStateChanged)).count() >= BUTTON_DELAY)
          {
        ...
        std::cout << "Player number " << *playerNumber << " too big. Maximum number is " << gameCtrlData.playersPerTeam << std::endl;
    }
  }
    ...
    while(udp && (size = udp->read((char*) &buffer, sizeof(buffer), from)) > 0)
      {
        if(size == sizeof(buffer) &&
           !memcmp(&buffer, GAMECONTROLLER_STRUCT_HEADER, 4) &&
  ...
  void close()
  {
    if(udp)
      delete udp;
    // TODO clear SPQR udp socket
 }

  // TODO DARIO create methods to initialize the gc on a different thread
...
public:
      ...
      teamNumber(0)
  {
    ...
    try
    {
      ...
      // TODO DARIO replace AL values with a vector of values
      // AL::ALValue params;
      // AL::ALValue result;
    ...
    {
      std::cout << "catch ... GameCtrl" << std::endl;
