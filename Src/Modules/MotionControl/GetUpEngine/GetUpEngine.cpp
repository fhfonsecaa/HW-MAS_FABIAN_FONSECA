...
{
  //std::cout << "[CEK DEBUG] angleY " << theInertialData.angle.x() << "| " <<theInertialData.angle.y() << std::endl; 
#ifndef NDEBUG
...
void GetUpEngine::pickMotion(GetUpEngineOutput& output)
{
  initVariables();
  output.criticalTriggered = false;
  const float& bodyAngleY = theInertialData.angle.y().toDegrees();
  const float& bodyAngleX = theInertialData.angle.x().toDegrees();
  ...
  //std::cout << "[CEK DEBUG] angleY " << bodyAngleY << std::endl;
  switch(state)
  {
    case decideAction:
    {
