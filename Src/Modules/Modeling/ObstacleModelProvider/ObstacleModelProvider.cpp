...
void ObstacleModelProvider::propagateObstacles(ObstacleModel& obstacleModel) const
{
  ...
  {
    if((ob.seenCount >= minPercepts || debug) && i < 7 
      && ob.center.x()<=theFieldDimensions.xPosOpponentFieldBorder && ob.center.x()>=theFieldDimensions.xPosOwnFieldBorder
      && ob.center.y()<=theFieldDimensions.yPosLeftFieldBorder && ob.center.y()>=theFieldDimensions.yPosRightFieldBorder)
