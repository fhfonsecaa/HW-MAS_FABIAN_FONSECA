  ...
  lastTimePenaltyMarkSeen(0), lastTimeCirclePerceptSeen(0), validitiesHaveBeenUpdated(false)
{
  ...
  if (theRobotInfo.number != 1) {
      ...
      for(int i = 0; i < samples->size(); ++i){
            samples->at(i).init(Pose2f(0.f, 2200.f, 0.f), defaultPoseDeviation, nextSampleNumber++, 0.5f);

//          (i < (samples->size()/5));
//          switch (i % 5) {
//          case 2:
//              samples->at(i).init(Pose2f(-60_deg, 2166.f, 500.f), defaultPoseDeviation, nextSampleNumber++, 0.5f);
//              break;
//          case 3:
//              samples->at(i).init(Pose2f(-30_deg, 2700.f, 866.f), defaultPoseDeviation, nextSampleNumber++, 0.5f);
//              break;
//          case 4:
//              samples->at(i).init(Pose2f(30_deg, 2166.f, -500.f), defaultPoseDeviation, nextSampleNumber++, 0.5f);
//              break;
//          case 5:
//              samples->at(i).init(Pose2f(60_deg, 2700.f, -866), defaultPoseDeviation, nextSampleNumber++, 0.5f);
//              break;
//          default: //1 (3,6)
//              samples->at(i).init(Pose2f(0.f, 2200.f, 0.f), defaultPoseDeviation, nextSampleNumber++, 0.5f);
//              break;
...
#ifndef PENALTY_STRIKER_GOALIE  //VINCENZO
...
#ifdef PENALTY_STRIKER_GOALIE        //VINCENZO
  if(/*theGameInfo.state != STATE_PLAYING && theGameInfo.state != STATE_INITIAL*/
          (theCognitionStateChanges.lastGameState != STATE_PLAYING && theGameInfo.state == STATE_PLAYING)){
      samples = new SampleSet<UKFSample>(numberOfSamples);
      if (theRobotInfo.number != 1) { //TODO
          for(int i = 0; i < samples->size(); ++i){
                samples->at(i).init(Pose2f(0.f, 2200.f, 0.f), defaultPoseDeviation, nextSampleNumber++, 0.5f);
          }
      }
