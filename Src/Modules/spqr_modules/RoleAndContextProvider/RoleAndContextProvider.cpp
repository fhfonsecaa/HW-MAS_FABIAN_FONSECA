...
std::tuple<float,int> RoleAndContextProvider::minOpponent_BallDistance(){
    ...
    for(auto obstacle : theTeamPlayersModel.obstacles){
        if(obstacle.type == Obstacle::opponent){
            dist = theLibCodeRelease.distance( Pose2f(obstacle.center), Pose2f(theTeamBallModel.position) );
            if(dist < minDist ){
                minDist = dist;
            }
            if(dist < 1500.f){
...
std::tuple<float,int, int> RoleAndContextProvider::minTeammate_BallDistance(){
    ...
    for(auto teammate : theTeamData.teammates){
        dist = theLibCodeRelease.distance( teammate.theRobotPose, Pose2f(theTeamBallModel.position) );
        if(dist < minDist ){
            minDist = dist;
...
void RoleAndContextProvider::update(RoleAndContext& rac) {
    ...
    //////////////////// ROLE ///////////////////////////////////////
	switch(theRole.role){
        case Role::goalie:rac.myRole = 1; break;
        case Role::striker:rac.myRole = 2; break;
        case Role::defender:rac.myRole = 3; break;
        case Role::supporter:rac.myRole = 4; break;
        case Role::jolly:rac.myRole = 5; break;
        case Role::searcher_1:rac.myRole = 6; break;
        case Role::searcher_2:rac.myRole = 7; break;
        case Role::searcher_3:rac.myRole = 8; break;
        case Role::searcher_4:rac.myRole = 9; break;
