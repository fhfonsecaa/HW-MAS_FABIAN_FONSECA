...
void FreeCorridorsProvider::update(FreeCorridors& freeCorridors) {
  ...
  for (const auto& corridor : freeCorridors.corridors) {
    Pose2f startRel = theLibCodeRelease.glob2Rel(corridor.start.x(), corridor.start.y());
    Pose2f targetRel = theLibCodeRelease.glob2Rel(corridor.target.x(), corridor.target.y());
