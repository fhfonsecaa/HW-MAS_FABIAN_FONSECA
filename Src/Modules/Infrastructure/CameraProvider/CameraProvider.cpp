...
void CameraProvider::useImage(bool isUpper, unsigned timestamp, CameraInfo& cameraInfo, Image& image, NaoCamera* naoCam)
{
  ...
  if(naoCam->hasImage()){
    ...
    if(cameraInfo.camera == CameraInfo::upper)
      image.CV_image.create(UPPER_CAMERA_HEIGHT, UPPER_CAMERA_WIDTH, CV_8U);
    else
      image.CV_image.create(LOWER_CAMERA_HEIGHT, LOWER_CAMERA_WIDTH, CV_8U);
    int nCols = cv_height * cv_width;
    uchar* p  = image.CV_image.ptr<uchar>(0);
    ...
    for (int i = 0, j = 0; i<nCols; i+=2){
      ...
      std::string name;
      if (isUpper)
          name = std::to_string(theImage.timeStamp) + "_up.png";
      else
          name = std::to_string(theImage.timeStamp) + "_lw.png";
      if (cv::imwrite( name, image.CV_image )) {
          std::cerr << name << " salvata" << std::endl;
      } else {
