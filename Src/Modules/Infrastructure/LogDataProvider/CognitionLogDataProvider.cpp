...
void CognitionLogDataProvider::update(Image& image)
{
  ...
  cv::Mat ycbcrMat(image.height, image.width, CV_8UC3,cvScalar(0,0,0));

    for (int i=0; i<image.height; i++) {
        for (int j=0; j<image.width; j++) {
            //if( mask.at<uchar>(i,j) == 0 ){
            //    continue;
                //ycbcrMat.at<cv::Vec3b>(i,j)[0] = 0;
                //ycbcrMat.at<cv::Vec3b>(i,j)[1] = 0;
                //ycbcrMat.at<cv::Vec3b>(i,j)[2] = 0;
    ...
    cvtColor(image.CV_image, image.CV_image, CV_BGR2GRAY);
