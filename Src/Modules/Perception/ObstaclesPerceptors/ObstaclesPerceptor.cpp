...
//#define DETECT_OBS_POSE
        ...
        if (obstacleOnField.type == ObstaclesFieldPercept::ownPlayer  /*E L'OSTACOLO È COMPLETO!!*/){
            ...
            for(float x = left; x < right; x += xStep)
                ...
                float xStep = std::max(1.f, width * 0.1f);
            uint counter = 0;
            uint half_counter = 0;

            uint counter_2 = 0;
            uint half_counter_2 = 0;



            int lateralOffset = std::abs(right - left)*0.15;
            int verticalOffset = std::abs(obstacleInImage.bottom - obstacleInImage.top)*0.08;
            //float heightOffset = obstacleInImage.upper -

            const uint start = left - 2.3*lateralOffset;
            const uint end = right+ 2.3*lateralOffset;
            for(uint h = start; h < end; h += xStep){
                uint x = obstacleInImage.top + verticalOffset;
                uint y = obstacleInImage.left - lateralOffset;

                FieldColors::Color color = theECImage.colored[obstacleInImage.top+verticalOffset][h];
                if (color == FieldColors::white && ((h-start )< 0.28f*(end-start) || ((h-start) > 0.73f*(end-start)))){
                    CIRCLE("representation:BallPercept:image", h, obstacleInImage.top+verticalOffset, 3, 1, // pen width
                           Drawings::solidPen,  ColorRGBA::black, Drawings::solidBrush, ColorRGBA(0, 0, 255, 80));

                    counter++;

                    if (h - start < 0.5f*(right - left)) half_counter = counter;
                }

                color = theECImage.colored[obstacleInImage.top+2*verticalOffset][h];
                if (color == FieldColors::white && ((h-start )< 0.28f*(end-start) || ((h-start) > 0.73f*(end-start)))){
                    CIRCLE("representation:BallPercept:image", h, obstacleInImage.top+2*verticalOffset, 3, 1, // pen width
                           Drawings::solidPen,  ColorRGBA::black, Drawings::solidBrush, ColorRGBA(0, 0, 255, 80));

                    counter_2++;

                    if (h - start < 0.5f*(right - left)) half_counter_2 = counter_2;
                }
            }


            if (std::abs(counter - 2*half_counter) < 0.25*counter && std::abs(counter - 2*half_counter_2) < 0.25*counter_2){
                //two arms raised
                std::cout << "TWO ARM RAISED!!!!" << std::endl;
                obstacleOnField.obsPose = ObstaclesFieldPercept::twoArmsRaised;
                if(theFrameInfo.getTimeSince(lastSound) > 2000){
                    SystemCall::playSound("TwoArmsRaised.wav");
                    lastSound = theFrameInfo.time;
                }


            } else if (std::abs(counter - half_counter) < 0.25*counter && std::abs(counter_2 - half_counter_2) < 0.25*counter_2)
//             || (std::abs(counter - half_counter) < 0.25*counter && std::abs(counter_2 - half_counter_2) < 0.25*counter_2)
            {
                //one arm raised
                std::cout << "ONE ARM RAISED!!!!" << std::endl;
                obstacleOnField.obsPose = ObstaclesFieldPercept::oneArmRaised;
                if(theFrameInfo.getTimeSince(lastSound) > 2000){
                    SystemCall::playSound("OneArmRaised.wav");
                    lastSound = theFrameInfo.time;
                }

            }
//            std::cout << "counter: " << counter << "half counter: " << half_counter << std::endl;
        }
#endif
...
/*
bool isWhiteForVinc(const Vector2i& a, const Vector2i& b) {
  const u_int whiteCheckStepSize = 10;
