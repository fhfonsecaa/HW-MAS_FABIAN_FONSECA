  ...
  for(int i = 0; i < numOfRobots; ++i)
  {
    ...
    if(automatic && r.info.penalty == PENALTY_NONE && r.lastPenalty != PENALTY_NONE && r.simulatedRobot)
    {
      if(gameInfo.gamePhase == GAME_PHASE_PENALTYSHOOT)
      {
        ...
        { //REMOVED BY EMANUELE THIS PART IS RELATED TO OLD CHALLENGE
...
GameController::BallOut GameController::updateBall()
{
  ...
  if(!fieldDimensions.isInsideField(ballInnerEdge))
  {
    ...
    else
    {
      ...
      else if (std::abs(ballPos.x()) > fieldDimensions.xPosOpponentGroundline)
      {
        ballPos.x() = ballPos.x() > 0 ? fieldDimensions.xPosOpponentGroundline : fieldDimensions.xPosOwnGroundline;
      ...
      else
      {
        ...
        */ 
        ...
        if(ballPos.y() < 0)
          ...
          ballPos.y() = fieldDimensions.yPosRightSideline; // right throw-in line
        else
          //ballPos.y() = fieldDimensions.yPosLeftDropInLine; // left throw-in line
          ballPos.y() = fieldDimensions.yPosLeftSideline; // left throw-in line
